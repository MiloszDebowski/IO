...
class My_PizzaBuilder : public PizzaBuilder {
public:
	void buildDough() override
	{
		pizza.setDough("Ciasto");
	}
	void buildSauce() override
	{
		pizza.setSauce("Keczup");
	}
	void buildTopping() override
	{
		pizza.setTopping("Kurczak");
	}
	Pizza getPizza() const override { return pizza; }
private:
	Pizza pizza;
...
int main()
{
	...
	My_PizzaBuilder myPizzaBuilder; // Zmieniono nazwę zmiennej
	cook.makePizza(myPizzaBuilder);
	Pizza myPizza = myPizzaBuilder.getPizza();
	std::cout << "\nMaciej Serafin Pizza Configuration:" << std::endl;
